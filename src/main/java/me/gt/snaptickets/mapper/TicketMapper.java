...
public interface TicketMapper {
    ...
    @Insert("INSERT INTO tickets (ticketid, name, description, image, price, stock, eventDate) " +
            "VALUES(#{ticketId}, #{name}, #{description}, #{image}, #{price}, #{stock}, #{eventDate})")
    ...
    @Select("SELECT * FROM tickets WHERE tickets.eventdate > #{eventDate}")
    List<Ticket> getUpcoming(LocalDateTime eventDate);

    @Update("UPDATE tickets SET " +
            "name = #{name}, description = #{description}, " +
            "price = #{price}, " +
            "eventdate = #{eventDate}, updatedat = CURRENT_TIMESTAMP " +
